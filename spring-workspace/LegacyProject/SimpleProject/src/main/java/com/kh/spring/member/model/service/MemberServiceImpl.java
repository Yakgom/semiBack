...
public class MemberServiceImpl implements MemberService {
	...
	private final MemberValidater validator;
	...
	public MemberDTO login(MemberDTO member) {
		...
		MemberDTO loginMember = memberMapper.login(member); //  memberRepository.login(sqlSession, member);
		
		/*if(loginMember == null) {
			
			throw new UserIdNotFoundException("아이디 또는 비밀번호가 틀렸습니다");
		}
	...
	public void delete(String userPwd, HttpSession session) {
		 ...
		 * 쉬운데 어려움(복잡함)
		 * 
		 * 세팅 수업시간에 했던거 다 가져다 쓰세요
		 * 
		 * 스프링을 이용한 구현 ==> 숙제(주말까지) ==> 꼭 하기!!
		 * 
		 * DynamicWebProject 회원파트 ==> Spring 버전으로 다시 만들기
		 * 
		 * 화면 다 있음 테이블 다있음 SQL문 다있음 ==> Service단 및 예외처리를 신경써보자
		 * 
		 * 
		 * 
		 * 
