...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 톰캣 실행 -> web.xml 파싱 init() => service() => doGet()/doPost() => destroy() 서블릿
		 * 생명주기 / 싱글톤 패턴으로 객체 하나만을 사용 / 톰캣이 멀티스레드 처리(스레드풀)
		 */

		// System.out.println("진짜임?");
		/*
		 * Dynamic WebProject 진행 시 Servlet을 Controller로 사용 1. 데이터가공 2. 요청처리(Service호출)
		 * 3. 결과값 반환(응답화면지정)
		 * 
		 ...
		 * 인자값으로 두 개 넘어옴
		 * 
		 * 첫 번째 매개변수 HttpServeltRequest 타입에는 요청 시 전달된 내용들이 담김 => 요청 전송 방식, 요청 URL, 요청한
		 * 사용자의 정보, 사용자가 input요소에 입력한 값 등, ...
		 * 
		 * 두 번째 매개변수 HttpServeltResponse 타입은 요청 처리 후 응답할 때 사용하는 객체
		 * 
		 * 요청 처리 스텝
		 * 
		 * 1. 우선 요청을 처리하기 위해서 요청 시 전달된 값(사용자가 입력된값)들을 뽑는다. => key-value 세트로
		 * 담겨있음(name속성값=value속성값) => request의 Parameter라는 곳에서 전달값을 뽑아내야함
		 * 
		 ...
		 * 
		 * -request.getParameter("키값") : String (input요소에 적어놓은 name속성값) => 반환형이 String이기
		 * 때문에 다른 자료형으로 사용해야 한다면 Parsing해야함
		 *
		 * - request.getParameterValues("키값"); : String[] => 하나의 key값으로 여러 개의 value들을
		 * 받아야 할 경우
		 */
		...
		// "셀 수 없음" ""(텍스트상자가 비어있을 경우 빈 문자열) / null
		// "홍길동" /
