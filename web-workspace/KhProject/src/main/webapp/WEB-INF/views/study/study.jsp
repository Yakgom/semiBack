...
<body>
	...
	<pre>
	    ...
	    단점 : SEO(Search Engine Optimization 검색 엔진 최적화) 취약		 
	    	  브라우저 히스토리 관리 복잡
	    	  JavaScript 의존성
	    	  보안 취약점 증가 (XSS)
	...
	진짜로 자바스크립트 객체 XXXXXXXXXXXXXX 자바 스크립트 객체 모양으로 문자열을 만든거
	...
	웹 개발의 표준!! 데이터 형식
	REST API 의 기본 포맷!!
	설정파일 XML -> JSON (설정파일은 YAML(YML)이 인기 더 많음)
	</pre>
	...
	<pre>
		...
		* 주요 속성 
		- url  : 오청할 URL(필수로 작성) => form태그의 action속성
		- type : 요청 전송 방식(GET/POST... FETCH,PUT,DELETE,OPTION...)
		- data : 요청 시 전달값 ({키 : 밸류 , 키 : 밸류}) => form태그의 input요서 value값
