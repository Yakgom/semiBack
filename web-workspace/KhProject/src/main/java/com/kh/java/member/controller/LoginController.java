...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * */
		// 절차
		// 1) GET? POST? => 요청 방식이 POST방식이라면 인코딩 작업 => POST는 무조건해야함
		...
		if(loginMember != null) {
			...
			// 브라우저를 종료하기 전까지는 게속해서 유지하고 사용할 것이기 때문에
			// session에 담기
			
			// 스텝 1. session 의 Attribute로 사용자 정보 추가
			// session의 타입 : HttpSession
			// => 현재 요청 보내는 Client의 Session : 
...
//			request.getRequestDispatcher("/index.jsp").forward(request, response);
			...
			// localhost:4000/kh
			// sendRedirect : Client에게 URL 에게 다시 요청하게 함
			// response 객체를 이용
			// response.sendRedirect("/다시 요청 보낼 URL경로");
			session.setAttribute("alertMsg", "로그인의 성공!!");
		...
		request.setAttribute("msg", "로그인실패했다");
