...
import com.kh.start.member.model.service.TokenService;
...
public class AuthServiceImpl implements AuthService {
	...
	public Map<String, String> login(MemberDTO member) {
		...
		// 로그인 구현
		// 1. 유효성 검사(아이디 / 비밀번호 값이 들어왔는가 , 영어숫자인가,글자수가 괜찮은가) -> @Valid로 대체
		
		// 2. 아이디가 BOOT_MEMBER 테이블에 Member_ID컬럼에 존재하는 아이디인가 
		...
		try {
		 auth = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(member.getMemberId(),member.getMemberPwd()));
			...
			throw new CustomAuthenitcationException("아이디 또는 비밀번호를 확인하세요.");
		}
