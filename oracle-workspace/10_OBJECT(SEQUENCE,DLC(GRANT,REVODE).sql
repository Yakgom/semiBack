 ...
 * MAXVALUE 최대값 -> 시퀀스로 만들어낼수있는 최대값 지정 옵션
 * MINVALUE 최소값 -> 최소값 지정 옵션
 * CYCLE / NOCYCLE -> 최대값까지 돌았을때 다시 최소값으로 돌아올건지 (순환 여부 지정)
 * -------------------- 생략가능
...
INSERT 
       VALUES((SELECT SELECT MAX(EMP_ID)+1 FROM EMPLOYEE), );
*/
...
-- 수정 : ALTER --> START WITH는 변경 불가능 바꾸고싶으면 DROP하고 다시 CREATE
-- 삭제 : DROP

CREATE SEQUENCE SEQ_EID
START WITH 224;

-- 사원을 추가해야할 떄마다 수행하는 INSERT
INSERT
  INTO
       EMPLOYEE
       (
