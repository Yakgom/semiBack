...
-- NULL을 ("KBJ01"."TB_USER_NOT_NULL"."USER_NO") 안에 삽입할 수 없습니다
-- Error position: line: 161 pos: 36
...
DROP TABLE TB_USER_UNIQUE; -- 테이블 드랍
...
-- 제약조건명을 달아두면 문제가 생긴 원인은 조금 더 쉽게 유추할수있음 
...
CREATE TABLE TB_USER_CHECK(
	...
	USER_ID VARCHAR2(20) NOT NULL UNIQUE,
	USER_PWD VARCHAR2(20) NOT NULL,
	GENDER CHAR(3) CHECK(GENDER IN ('남','여')) -- TRUE일떄만 INSERT가능
);
...
CREATE TABLE TB_USER_DEFAULT(
	...
	ENROOL_DATE DATE DEFAULT SYSDATE NOT NULL
);
...
INSERT INTO TB_USER_PK
VALUES (1,'user01','12344',NULL,NULL);
 ...
 * [ 표현법 ]
 * - 컬럼 레벨 방식
 * 컬러명 자료형 REFERENCES 참조할테이블명(참조할컬럼명)
 * 
 * - 테이블 레벨 방식
 * 
...
INSERT INTO TB_USER_CHILD VALUES(5,'user05','pass05','G5');     
...
-- 현재 변경하려고하는 컬럼의 값과 완전히 다른 타입이거나 작은 크기로는 변경이 불가능함!
...
-- 3_1) 컬럼명 바꾸기 : ALTER TABLE 테이블명 RENAME COLUMN 원래 컬럼명 TO 바꿀컬럼명
