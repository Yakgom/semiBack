...
--> JOIN 을 통해 연결고리에 해당하는 컬럼만 매칭시킨다면 마치 하나의 조회 결과물처럼 조회 가능
...
SELECT
  ...
  FROM
       CLASS_ROOM, STUDENT
 WHERE 
       CLASS_ROOM.CLASS_ID = STUDENT.CLASS_ID; -- 2절 끝       
/*
 * < JOIN >
 * 
 ...
 * 
 * JOIN 크게 "오라클 전용구문" 과 "ANSI(미국국립표준협회)구문"으로 나뉜다.
 * 
 * ===================================================================
...
-- 일치하지 않는 값은 조회에서 제외
-- DEPT_CODE가 NULL인 사우너 2명은 데이터가 조회되지 않는다.
...
-- 컬럼명이 동일할 경우 USING 구문을 이용할수가 있다 동등비교연산자를 기술하지 않음
...
NATURAL
  JOIN
       JOB;
-- 두 개의 테이블을 조인하는데 운 좋게도 두 개의 테이블의 일치하는 컬럼이 딱 하나있었다.
/*
 * < QUIZ >
 * 
 */		
...
-- 조건과는 상관없이 왼편에 기술한 테이블의 데이터는 전부 조회(이리하는 값을 못찾더라도 조회)
...
-- 일치하는 컬럼이 존재하지 않더라도 오른쪽 테이블의 데이터는 무조건 조회
...
SELECT * FROM DEPARTMENT;  -- DEPT_TITLE   DEPT_ID             LOCATTION_ID
...
SELECT 
     ...
     , LOCAL_NAME
  FROM
       EMPLOYEE E,DEPARTMENT,JOB J,LOCATION
 WHERE
       DEPT_CODE = DEPT_ID(+)
   AND
       LOCATION_ID = LOCAL_CODE(+)
...
-- 부서코드가 D1,D2,D5 인 부서의 급여합계를 조회하고 싶다
