 ...
 * 
 * [ 표현법 ]
 * SELECT 조회하고자 하는 컬럼,조회하고자 하는 컬럼, FROM 테이블명;
 * 
...
-- EMPLOYEE 테이블명으로부터 사원명,월급,보너스,보너스를 포함한 연봉((월급*보너스+월급)) 
SELECT 
	   EMP_NAME
	 , SALARY 
	 , BONUS
  ...
  FROM
       EMPLOYEE;
--> 사술연산 과정에서 NULL 값이 존재할 경우 산술연산의 결과도 NULL이 된다.
 ...
 * [ 표현법 ]
 * 
 * 컬럼명 AS 별칭 , 컬렴명 AS "별칭" , 컬럼명 "별칭" , 컬럼명 별칭
 * 
 * 
...
-- EMPLOYEE 테이블로부터 부서코드가 D9 가 아닌 사원들의 사원명 , 전화번호 조회
...
-- EMPLOYEE 테이블에서 급여가 350 이상 500이하인 사원들의 사번 이름 직급코드(JOB_CODE)
SELECT
       EMP_ID
     , EMP_NAME
     , JOB_CODE
 ...
 * 비교대상컬림 LIKE '__A' => 컬럼값에 'A' 앞에 무조건 2글자가 있어야만 패턴에 만족
 * 
 */
...
-- 테이블로부터 보너스를 받지 않는(보너스가 없는) 사원들의 사원명, 보너스 조회 
...
/*
 * < 연결연산자 || >
 * 여러개의 컬럼 값들을 마치 하나의 컬럼인것처럼 연결 시켜주는 연산자
 * 컬럼값 또는 리터럴(문자열)을 전부 다 합칠 수 있음
 * 
 * Sysout.out.println(num + "sdafda"); 느낌
 * 
 *  
 */
SELECT 
...
-- EMPLOYEE 테이블로부터 부서코드가 D6 이거나 D8 또는 D5인 사원명,부서코드 조회
...
-- 첫 번째로 제시한 정렬기준의 컬럼값이 동일할 경우 다음 정렬 기준을 명시할 수 있음
...
SELECT
 ...
 ORDER
    BY
       "연봉"; -- 실행하는 순서가 1 FROM 2 WHERE 3 SELECT 이기 때문에 WHERE에서는 별칭을 못쓴다. 
       -- 2; 권장하지 않는 (비추천) 방법 컬럼의 순서로 정렬 
