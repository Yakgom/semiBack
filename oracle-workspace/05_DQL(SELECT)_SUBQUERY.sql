 ...
 * 서브쿼리의 분류
 * 
 * 서브쿼리를 수용한 결과가 몇 행 및 열이냐에 따라서 분류됨
 * 
 * - 단일행 [단일열] 서브쿼리 : 서브쿼리 수행 결과가 딱 1개일 경우
 * - 다중형 [단일열] 서브쿼리 : 서브쿼리 수행 결과가 여러 행일 때
 * - [단일열]다중열 서브쿼리  : 서브쿼리 수행 결과가 여러 열일 때
 * - 다행형 다중열 서브쿼리   : 서브쿼리 수행 결과가 여러 행, 여러 열 일때
 * 
 * 
 ...
 * 일반 연산자 사용(=, != , < , > ....)
 */
-- 전직원의 평균 급여보다 적게 받는 사원들의 사원명 , 전화번호 조회
1. 평균 급여 구하기
SELECT 
       AVG(SALARY)
  FROM
...
SELECT
 ...
 WHERE
       SALARY = (SELECT
  						 ...
  						 FROM
       						EMPLOYEE);
-- 안준영 사원의 급여보다 더 많은 급여를 받는 사원들의 사원명,급여조회
SELECT
       SALARY
  FROM
       EMPLOYEE
...
----------------------------------------------------------------
-- 우리가 지금 하고 있는 작업이 어떤걸 하고 있는 건가요?
-- DB에다가 주고받고 --> 컴퓨터 작업
/*
 * 2. 다중 행 서브쿼리
 ...
 * 그렇기 때문에 JOIN으로 대체하는 것이 일반적으로 성능상 유리함
 * 단, 캐싱이 되기 때문에 동일한 결과에 대해선 성능상 JOIN보다 뛰어날 수도 있음
 * 스칼라 쿼리는 반드시 단 한개의 값만 반환해야함
 * 
 */
