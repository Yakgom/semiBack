 ...
 * 
 *    - 단일행 함수 : N개의 값을 읽어서 N개의 결과를 반환(매 행마다 함수 결과를 반환)
 *    - 그룹 합수 
 * 
 */
...
SELECT 
  ...
  FROM 
       DUAL; -- 앞에서부터 3번째 'B'가 앞에서부터 몇 번째인지
 ...
 * LPAD / RPAD
 * 
 * - LPAD / RPAD(STR, 최정적으로 변환할 문자의 길이(바이트),패딩할문자)
 ...
 * TRIM
 * 
 * -TRIM(BOTH / LEADING / TRAILING은 '제거하고자하는문자' FROM STR)
 ...
 */ 
 ...
 SELECT
 		  FLOOR(123.45 * 10) / 10 -- 뒤에 한자리만 버리고싶다 
   FROM
        DUAL;
 ...
 SELECT
 		  EMP_NAME
 		, CONCAT(FLOOR(SYSDATE-HIRE_DATE),'일') AS "근무일자"
   FROM
        EMPLOYEE
  WHERE
 ...
 -- NEXT_DAY(DATE,요일) : 특정날짜에서 가장 가까운 요일을 찾아 그날짜를 봔한
 SELECT
 ...
 -- 언어 변경 ALTER SESSION SET NLS_LANGUAGE = AMERICAN; // KOREAN
...
-----------------------------------------------------------------------------------------
 ...
 SELECT
      ...
      , TO_CHAR(SYSDATE,'DDD') -- 일년 기준(1월1일로부터)
   FROM
        DUAL;
...
SELECT
  ...
  FROM
       EMPLOYEE;
-- 직급코드(JOB_CODE)가 'J7'인 사원들은 15프로 인상
-- 'J6' 인 사원은 20프로 인상
-- 'J5' 인 사원은 30프로
-- 나머지 직급인 사원들은 5프로
SELECT
     ...
     , JOB_CODE
     , DECODE(JOB_CODE,'J7'
     						 ,(SALARY+SALARY *0.15),'J6'
     						 ,(SALARY+SALARY*0.2),'J5'
     						 ,(SALARY+SALARY*0.3)
...
SELECT
       MIN(SALARY) AS "제일 작은 급여"
     , MIN(EMP_NAME) AS "제일 이름이 빠른사람"
     , MIN(HIRE_DATE) AS "제일 빠른 입사일"
  FROM
       EMPLOYEE;
...
SELECT
		 MAX(SALARY) AS "가장 큰 급여"
     , MAX(EMP_NAME) AS "가장 이름이 느린사람"
     , MAX(HIRE_DATE) AS "가장 느린 입사일"
  FROM
       EMPLOYEE;
