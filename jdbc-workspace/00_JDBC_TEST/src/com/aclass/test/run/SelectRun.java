		...
		*/
		...
		try {
			...
			// 코드의 변경 없이 데이터베이스를 연결할 수 있도록 
			 ...
			 //System.out.println("번호 : " + studentId + ", 이름 : " + studentName + "날짜 : " + enrollDate);
			 
			 
			 while(rset.next()) {
				 // 커서를 조작했을 때 행이존재한다면 true / 행이 존재하지 않는다 false 
				 	int studentId = rset.getInt("STUDENT_ID");           // 숫자 컬럼
				    String studentName = rset.getString("STUDENT_NAME"); // 문자열 컬럼
				    Date enrollDate = rset.getDate("ENROLL_DATE");// 날짜 컬럼
				 
				    sb.append(studentId + " | " + studentName + " | " + enrollDate);
				    sb.append("\n");
			 }
			...
			System.out.println("jdbc:oracle:thin:@115.90.212.20:10000XE 오타 없나요?");
