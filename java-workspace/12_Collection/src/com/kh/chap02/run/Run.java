...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * => 만약 요소가 추가되면서 크기를 넘어서는 상황이 발생하면 알아서 확장된다.
		 ...
		 *               -> ArrayList
		 *  
		 *  - Set 계열 : 다루고자 하는 값(Value)만 저장 / 저장 시 순서 유지X , 중복값 허영 X
		 *  			=> HashSet , TreeSet
		 *  			-> HashSet
		 *  
		 *  - Map 계열 : 키(key) - 값(Value)세트로 저장 / 저장 시 순서 유지 X , 키 중복 허용X / 값 중복O
		...
		// add 메소드를 호출 시 오버로딩된 형태의 메소드를 사용
		// add(int index,E e) : List의 index에 전달한 요소를 추가
		plants.add(0,"식물시작~");
		plants.add(3, new Plant("관엽","홍콩야차"));
		
	
		// 중간위치에 요소를 추가했을 때 기존의 요소들을 한 번씩 밀어주는 작업이 내부적으로 구현되어있음
		...
		// 2. 요소를 수정하는 메소드
		// set(int, index ,E e )
		// List에 값을 전달된 요소 (e)로 set해주는 메소드
		plants.set(0,"시~~~작~");
		...
		for(Object a : plants) {
			System.out.println(a.toString()); // 동적바인딩 
		}
		
		
		// 6. 리스트 자르기
		// subList(int index1, int index2) : List
		// index1 부터 index2까지의 요소를 추출해서 새로운 리스트를 반환
