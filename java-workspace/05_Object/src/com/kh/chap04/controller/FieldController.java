 ...
 * - (local) variable : 클래스 영역 내부에 특정한 구역(){}
 *                      => method , for , if 등등...
 * 
 * 
 * 1. 전역 변수의 역할을 하는 친구들
 ...
 *  
 * 	- 클래스 변수 (정적,스태틱) : static이라는 예약어가 붙어있는 변수
 * 						   생성 시점 : 프로그램 시작과 동시에 static영역에 올라감
 * 						   소멸 시점 : 프로그램 종료되면 소멸 
 * 
 * 2. 지역 변수 
 *			생성 시점 : 특정 연역( { } ) 내부에서 선언되는 시점에 메모리 영역에 할당 -> Stack
 * 			소멸 시점 : 특정 영역( { } )이 종료될 때
 * 
 * 
 * */
...
public class FieldController {
	...
	public void checkVariable(int prameter) {
		
		// local (지역변수)할당 및 초기화
		int local = 1; // 지역변수 특 : 초기화 안하면 사용할 수 없음
		
		System.out.println(global); //필드 멤버변수
		System.out.println(prameter); // 매개변수(지역변수)
