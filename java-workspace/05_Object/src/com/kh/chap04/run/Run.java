...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 목표 : str출력하기
		/*
		 * StaticController sc = new StaticController(); System.out.println(sc.str);
		 */
		System.out.println(StaticController.str);
		 ...
		 *클래스 다이어그램 표기
		 * (+) public => 어디서든(같은 패키지 , 다른 패키지 , 클래스 내/외부 포함 전부)
		 * 
		 * (#) protected => 같은 패키지라면 무조건 접근 가능 !
		 * 			 => 다른 패키지라면 상속구조인 클래스에서만 접근 가능 
		 * 			 => 상속이라는 개념은 05_Object / 06_ObjectArray 끝나고 07번에서 배움
		 * 			
		 * (~) default(package friendly)   => 오로지 같은 패키지에서만 
		 * 
