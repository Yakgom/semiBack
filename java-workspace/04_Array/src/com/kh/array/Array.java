...
public class Array {
	...
	/*
	 * 배열(Array) : 하나의 배열에 여러 개의 값을 담을 수 있음 단, "같은 자료형의 값을"만 담을 수 있음 동종
	...
	public void method1() {
		...
		// 임의의 정수 5개를 선언하고 초기화를 진행함
		/*
		 * int num1 = 12; int num2 = 2; int num3 = 99; int num4 = 34; int num5 = 343;
		 * 
		 ...
		 * 배열 선언 방법
		 * 
		 * 1) 자료형 배열식별자[]; 2) 자료형[] 배열식별자; -> 이 방법을 사용할 것
		 * 
		 * 
		 */
	...
	public void methodA() {
		...
		/*
		 * 기본자료형 : boolean , char , byte, short , int , long , float , double => 변수 공간에
		...
		/*
		 * String[] khAcademy; khAcademy = new String[5]; System.out.println(khAcademy);
		 * 
		 * khAcademy[0] = "인포"; khAcademy[1] = "A강의장"; khAcademy[2] = "B강의장";
		 * khAcademy[3] = "C강의장"; khAcademy[4] = "사무실";
		 * 
		 * // c강의장 출력
		 * 
		 * System.out.println("-----------반복문------------");
		 * 
		 * for(int index = 0; index<5 ; index++){ System.out.println(khAcademy[index]);
		 * }
		 */
	...
	public void lotto() {
		...
		/*
		 * for(int i=0; i<lottoNum.length; i++) { System.out.print(lottoNum[i] + " "); }
		 */
	...
	public void methodB() {
		...
		// 5개의 요소를 담을 수 있는 String의 배열을 선언 및 할당하시오.
		// 동물 5총사
	...
	public void methodC() {
		 ...
		 * 
		 * if(nums[0] < nums[1] && nums[1] < nums[2]) { System.out.println("최소값은 : " +
		 * nums[0] + " 입니다."); }
		 */
		...
		/*
		 * Arrays.sort(nums); System.out.println(nums[0]);
		 */
	...
	public void methodD() {
		...
		/*
		 * 
		 * 연결이 끊긴 기존의 배열은 일정 시간이 지나면 G.C가 알아서 삭제를 해준다. : 자동 메모리 관리
		 * 
		...
		// 가르키고 있는 대상이 null 인데 주소가 없는데 어디에 접근함?
	}
...
//		System.out.println(Arrays.toString(copy));
		...
		/*
		 * 네이밍 컨벤션 == 첫 글자가 대문자 클래스 / 인터페이스
		 * arraycopy() ==> System클래스
		 * 
		 * 
		 * 
		 * System.arraycopy(원본 배열 식별자, 원본배열에서 복사를 시작할 인덱스,
		 * 					복사본 배열 식별자, 복사본 배열에서 복사가 시작할 인덱스,
		 * 					복사할 개수);
		 * 
		 * 
		 * 
